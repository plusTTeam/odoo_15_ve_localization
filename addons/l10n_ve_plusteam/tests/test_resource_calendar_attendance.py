...
class TestResourceCalendarAttendance(TransactionCase):
    ...
    def test_update_translations(self):
        ...
        attendance = self.env["resource.calendar.attendance"].search([("name", "=", "Lunes por la mañana")])
        self.assertEqual(attendance.name, "Lunes por la mañana", msg="Day not found")
        attendance = self.env["resource.calendar.attendance"].search([("name", "=", "Martes por la mañana")])
        self.assertEqual(attendance.name, "Martes por la mañana", msg="Day not found")
        attendance = self.env["resource.calendar.attendance"].search([("name", "=", "Miércoles por la mañana")])
        self.assertEqual(attendance.name, "Miércoles por la mañana", msg="Day not found")
        attendance = self.env["resource.calendar.attendance"].search([("name", "=", "Jueves por la mañana")])
        self.assertEqual(attendance.name, "Jueves por la mañana", msg="Day not found")
        attendance = self.env["resource.calendar.attendance"].search([("name", "=", "Viernes por la mañana")])
